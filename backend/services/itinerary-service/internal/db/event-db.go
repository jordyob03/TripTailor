...
func AddEvent(DB *sql.DB, event models.Event) (int, error) {
	insertEventSQL := `
...
func GetEvent(DB *sql.DB, eventID int) (models.Event, error) {
	...
	if err != nil {
		...
		return models.Event{}, fmt.Errorf("failed to retrieve event: %w", err)
	}
